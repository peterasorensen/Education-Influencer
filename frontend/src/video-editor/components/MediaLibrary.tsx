...
import { useEditorStore, type MediaItem, type TimelineClip, type TimelineTrack } from '../store';
...
// Utility function to generate thumbnail from video
...
// Utility function to generate thumbnail from image
const generateImageThumbnail = (file: File): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      // Create thumbnail (max 300px width/height while maintaining aspect ratio)
      const maxSize = 300;
      let width = img.width;
...
// Utility function to extract media metadata
const extractMediaMetadata = (file: File): Promise<{
  duration: number;
  width?: number;
  height?: number;
}> => {
    ...
    >
      ...
      {mediaItems.length === 0 ? (
        <EmptyState>
          ...
          <p>Click the button above or drag files here to import</p>
        </EmptyState>
      ) : (
        <MediaList>
          {mediaItems.map((item) => (
