...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  removeMediaItem: (id) =>
    set((state) => {
      ...
      // Revoke blob URL to free memory (web-specific cleanup)
      const mediaItem = state.mediaItems.find((item) => item.id === id);
      if (mediaItem?.filePath.startsWith('blob:')) {
        URL.revokeObjectURL(mediaItem.filePath);
      }
      if (mediaItem?.thumbnail?.startsWith('blob:')) {
